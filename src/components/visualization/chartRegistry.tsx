...
export const CHARTS: Record<ChartId, ChartDef> = {
  iq: {
    title: 'I/Q waveform', Plot: memo(IQPlot), zoomable: true,
    meta: (p) => [['Sample rate', p ? `${p.sampleRate} MS/s` : dash], ['Peak', p ? `${p.peakPower.toFixed(1)} dBm` : dash]],
  },
  spectrum: {
    title: 'FFT / spectrum', Plot: memo(SpectrumPlot), zoomable: true,
    meta: (p) => [
  ...
  waterfall: {
    title: 'Spectrogram', Plot: memo(SpectrogramPlot), zoomable: false,
    meta: (p) => [['Span', '1.85–3.05 MHz'], ['Duration', p ? `${p.duration.toFixed(2)} s` : dash]],
  },
  constellation: {
    title: 'Constellation', Plot: memo(ConstellationPlot), zoomable: false,
    meta: (p) => [['EVM', p ? `${p.evm.toFixed(1)} %` : dash], ['Phase offset', p ? `${p.phaseOffset.toFixed(1)}°` : dash]],
  },
  eye: {
    title: 'Eye diagram', Plot: memo(EyePlot), zoomable: false,
    meta: (p) => [['Symbol rate', p ? `${p.symbolRate} kSym/s` : dash]],
  },
  'freq-time': {
    title: 'Frequency vs time', Plot: memo(FreqPlot), zoomable: true,
    meta: (p) => [['Carrier offset', p ? formatCFO(p.cfo) : dash]],
  },
  amplitude: {
    title: 'Amplitude vs time', Plot: memo(AmplitudePlot), zoomable: true,
    meta: (p) => [['Avg power', p ? `${p.averagePower.toFixed(1)} dBm` : dash], ['Peak', p ? `${p.peakPower.toFixed(1)} dBm` : dash]],
  },
  'phase-time': {
    title: 'Phase vs time', Plot: memo(PhasePlot), zoomable: true,
